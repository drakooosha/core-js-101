...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    if ((str.slice(0, i) + str.slice(i + 1)).indexOf(str[i]) === -1) {
      return str[i];
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 1; i < ccnArr.length; i += 2) {
    const firstNum = Number(String(ccnArr[i] * 2)[0]);
    const secondNum = Number(String(ccnArr[i] * 2)[1]);
    ccnArr[i] = ccnArr[i] * 2 > 9 ? firstNum + secondNum : ccnArr[i] * 2;
  }
  const result = ccnArr.reduce(((elem, sum) => Number(sum) + Number(elem)), 0);
  return result % 10 === 0;
}
